...
public record CoordinateDetailsResponse(
        @Schema(description = "옷 태그 정보 리스트") List<Payload> payloads) {
    @Schema(name = "CoordinateDetailsResponsePayload")
    ...
    public static CoordinateDetailsResponse from(List<CoordinateCloth> coordinateClothes) {
        List<Payload> payloads =
                coordinateClothes.stream()
                        .map(
                                cc ->
                                        new Payload(
                                                cc.getId(),
                                                cc.getLocation().getLocationX(),
                                                cc.getLocation().getLocationY(),
                                                cc.getRatio(),
                                                cc.getDegree(),
                                                cc.getOrder(),
